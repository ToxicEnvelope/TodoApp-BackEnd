...
class PathUtil(object, metaclass=Singleton):
    ...
    def get_logs_path():
        if platform().lower().startswith('win'):
            final_path = PathUtil.resource_path(r'logs\runtime_%s.log' % datetime.datetime.now().isoformat())
            logs_dir = r'%s' % final_path
        else:
            final_path = PathUtil.resource_path('logs/runtime_%s.log' % datetime.datetime.now().isoformat())
